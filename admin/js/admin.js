...
//******************************Hide again after use, and run admin again ONLY after clearing cache (very important).
...
console.log(ms);
		...
		//Send to update the news-item order on the website front page 
		$("#blockplacement-button").click(function() {
			var nextPosition=$('#blockplacement-input').val();
			var storeOriginal ='newsItem'+pageNumber;
			var iplus1=1;
			var iminus1=1;
			var newsItemAtPositionIplus1="";
			var newsItemAtPositionIminus1="";
			console.log("nextPosition = "+nextPosition);
			console.log("currentPosition = "+currentPosition);
			nextPosition=parseInt(nextPosition);
			currentPosition=parseInt(currentPosition);

			if($("#page"+nextPosition).text()===""){  //going to empty space... dont move other newsItems
				...
				$("#page"+currentPosition).text(""); //so next newsItem can move to vacated space
				for (var i=currentPosition;i<nextPosition;i++){
					iplus1=parseInt(i)+1  // get the newsItem at position i+1 - i is a string for some wierd reason
					newsItemAtPositionIplus1=$("#page"+iplus1).text();
					...
					console.log("looking at "+newsItemAtPositionIplus1+ " currently at position "+iplus1);
					$("#page"+iplus1).text("");
					var data= {"_id":newsItemAtPositionIplus1};
					data['savedBlockPlacement']=i.toString();
					console.log("newsItem "+newsItemAtPositionIplus1+ " now has position "+i);
					socket.emit('updateNews',data,newsItemAtPositionIplus1);
				}
			}else if(nextPosition<currentPosition){
				$("#page"+currentPosition).text(""); //so next newsItem can move to vacated space
				for (var i=currentPosition;i>nextPosition;i--){
					iminus1=parseInt(i)-1  // get the newsItem at position i+1 - i is a string for some wierd reason
					newsItemAtPositionIminus1=$("#page"+iminus1).text();
					matches=newsItemAtPositionIminus1.match(/\d+$/);//extract number;
					if (matches) {
			    			newsItemAtPositionIminus1 = matches[0];
					}
					newsItemAtPositionIminus1="newsItem"+newsItemAtPositionIminus1;
					$("#page"+iminus1).text("");
					var data= {"_id":newsItemAtPositionIminus1};
					data['savedBlockPlacement']=i.toString();
					socket.emit('updateNews',data,newsItemAtPositionIminus1);
				}
			}
			var saveAs ='newsItem'+pageNumber;
			var data= {"_id":saveAs};
			data['savedBlockPlacement']=nextPosition;
			socket.emit('updateNews',data,saveAs);

			var delayInMilliseconds = 2000; //2 second
			setTimeout(function() {
				insertValues();
				var newdelayInMilliseconds = 2000; //2 second
				setTimeout(function() {
						orderPages();
				}, newdelayInMilliseconds);
			}, delayInMilliseconds);
		});

		$("#maintextcomment-button").click(function() {
			var currentBlock=pageNumber;
			var saveAs ='newsItem'+currentBlock;
			var data= {"_id":saveAs};
			console.log("6saveAs = "+saveAs);
			data['savedSpareTwo']=document.getElementById("maintextcomment-input").value;
			console.log("saveAs = "+saveAs);
			document.getElementById("maintextcomment-input").value="updating...";
			socket.emit('updateNews',data,saveAs);
		});
...
//console.log("allData = "+allData);
				...
				if(j==='savedSpareOne'){//YouTube embed
					document.getElementById("youtube-input").value=allData;
					allNewsStorage[saveAs]['savedSpareOne']=allData;
				}
				if(j==='savedSpareTwo'){//single-line comment
console.log("savedSpareTwo = "+allData);
					document.getElementById("maintextcomment-input").value=allData;
					allNewsStorage[saveAs]['savedSpareTwo']=allData;
				}
			}
		})
		...
		function insertValues(){
			...
			console.log(fred+"savedNewsTitle : "+allNewsStorage[fred]["savedNewsTitle"]);
			console.log(fred+"savedNewsText : "+allNewsStorage[fred]["savedNewsText"]);
			console.log(fred+"savedNewsImage : "+allNewsStorage[fred]["savedNewsImage"]);
			console.log(fred+"savedNewsTimepassed : "+allNewsStorage[fred]["savedNewsTimepassed"]);
			console.log(fred+"savedBlockPlacement : "+allNewsStorage[fred]["savedBlockPlacement"]);
			console.log(fred+"savedShortenedNewsText : "+allNewsStorage[fred]["savedShortenedNewsText"]);
			console.log(fred+"savedSpareOne : "+allNewsStorage[fred]["savedSpareOne"]);
			console.log(fred+"savedSpareTwo : "+allNewsStorage[fred]["savedSpareTwo"]);
				...
				document.getElementById("maintextcomment-input").value=allNewsStorage[fred]["savedSpareTwo"];
		...
		//console.log("$(#page+blockMovedTo).text() = "+$("#page"+blockMovedTo).text());
						...
						$("#page"+blockMovedTo).append(fred).css("display","inline-block");
						//console.log(fred);
					}else{
			...
			//if(loginID==="newsItem"){
			//	$(".sub-arrow").css("display","inline-block")
			//}
		}
	}catch(e){
